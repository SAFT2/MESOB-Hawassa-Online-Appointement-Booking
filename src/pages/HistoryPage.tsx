...
export default function HistoryPage() {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      .from("appointments")
      .select("id, reference, appointment_date, slot_window, queue_number, status, institutions(name), services(name)")
      .eq("user_id", user.id)
      .order("appointment_date", { ascending: false })
      .order("slot_window");
    setLoading(false);
    if (error) { toast.error(error.message); return; }
    setAppts((data ?? []) as unknown as AppointmentRow[]);
  };
        ...
        {authLoading || loading ? (
          ...
          <div className="mt-10 rounded-xl border bg-card p-8 text-center">
            ...
            <Link to="/book" className="mt-4 inline-block">
              <Button>{t("history_book_now")}</Button>
            </Link>
          </div>
        ) : (
